...
public:
    ...
    vector<string>& get_seq_str() ;      //TODO retornar ref?
    unsigned get_seq_length()const;
    map<string,map<unsigned,vector<shared_ptr<TIRP>>>>& get_definitive_ones_indices_dict();  //TODO retornar ref?
    ...
    unsigned update_tirp_attrs(const string &seq_id, unsigned f_eid, VertTirpSidList &f_sidlist, bool mine_last_equal, const Allen &ps, const vector<shared_ptr<TIRP>> &tirps_to_extend, eps_type eps, time_type  min_gap, time_type  max_gap, time_type max_duration, support_type min_ver_sup, const map<string,shared_ptr<TIRPstatistics>>  &father_discovered_tirp_dict, int min_confidence,double &temps);
    void first_sorted_extend(const string &sid, unsigned eid, const vector<shared_ptr<TIRP>> &new_tirps);

private:
    vector<string> seq_str;
    map<string,map<unsigned,vector<shared_ptr<TIRP>>>> definitive_ones_indices_dict; //TODO potser list (first_sorted_extend() fa inserció a davant)
