...
Allen::Allen(bool dummy_calc, bool trans, eps_type eps, string calc_sort) {
    ...
    if ( !dummy_calc ) {
        ...
        pair< shared_ptr<PairingStrategy>,shared_ptr<vector<string>> > aux_pair = AllenRelationsEPS::get_pairing_strategy(this->calc_sort);
        ...
        if ( this->eps > 0 ){
            for ( const auto &it : AllenRelationsEPS::trans_table ){
                if ( it.second.size() == 1){
                    ...
                    this->sorted_trans_table.insert(pair<string,Relation>(it.first,Relation(AllenRelationsEPS::get_pairing_strategy(it.second.getString()) )));
                }
            }
        }
        else{
            for ( const auto &it : AllenRelationsEPS::trans_table_0 ){
                if ( it.second.size() == 1){
...
Relation& Allen::get_possible_rels(char a, char b) {
    if ( this->dummy_calc ) {
        if ( this->eps > 0 )
            return AllenRelationsEPS::trans_table.at(string({a, b}));
        else
            return AllenRelationsEPS::trans_table_0.at(string({a, b}));
    }
    else
...
Allen::calc_rel(const TI* a, const TI* b, eps_type eps, time_type min_gap, time_type max_gap, shared_ptr<PairingStrategy> rels_arr, shared_ptr<vector<std::string>> gr_arr)const {
    ...
    if ( this->dummy_calc ){
        ...
        for ( char r : {'l','f','s','o','b','c','m','e'} ){
            rel = AllenRelationsEPS::rel_func_dict.at(r)(a, b, eps, min_gap, max_gap);
            if ( rel[1] != -2)
                final_rel = rel;
        }
        ...
        for ( const auto &sentence : rels_arr->get_ps() ){
            if ( gr_arr->operator[](i) != NONE ){
                if ( AllenRelationsEPS::cond_dict.at(gr_arr->operator[](i))(a, b, eps, min_gap, max_gap) ){
                    for ( const Node &words : sentence ){
                        if ( words.dif ){
                            if ( AllenRelationsEPS::cond_dict.at("mo")(a, b, eps, min_gap, max_gap)) {
                                for (const char &w: words.l) {
                                    rel = AllenRelationsEPS::ind_func_dict.at(w)(a, b, eps, min_gap, max_gap);
                                    if (rel[1] > -2)
                                        return rel;
                                }
                            }
                            ...
                            rel = AllenRelationsEPS::ind_func_dict.at(words.cont)(a, b, eps, min_gap, max_gap);
                            if ( rel[1] > -2 )
                                return rel;
                        }
                    }
                ...
                rel = AllenRelationsEPS::ind_func_dict.at('b')(a, b, eps, min_gap, max_gap);
                if ( rel[1] > -2)
                    return rel;
            }
            i++;
    ...
    if ( this->dummy_calc ) {
        ...
        if ( possible_rels.size() == 1){
            ...
            if ( first_r == 'b' ){ // Special case with gap
                int* calculated = AllenRelationsEPS::ind_func_dict.at('b')(a, b, eps, min_gap, max_gap);
                ...
                return AllenRelationsEPS::predefined_rels.at(first_r);
            }
        }
        else{
            int* calculated;
            for ( const char &r : possible_rels.getString() ){
                calculated = AllenRelationsEPS::rel_func_dict.at(r)(a, b, eps, min_gap, max_gap);
                if ( calculated[1] != -2 )
                    return calculated;
            }
            return this->calc_rel(a, b, eps, min_gap, max_gap,shared_ptr<PairingStrategy>(nullptr), shared_ptr<vector<string>>(nullptr));
